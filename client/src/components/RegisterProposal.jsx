...
const RegisterProposal = ({ walletAddress, idlWithAddress, getProvider }) => {
  ...
  const registerProposal = async () => {
    ...
    //Feth treasury config to get treasury token account
    ...
    const proposalTokenAccount = await getAssociatedTokenAddress(xMintPda, provider.wallet.publicKey);

    const deadlineTimestamp = new anchor.BN(Math.floor(new Date(deadline).getTime() / 1000));
    const stakeRaw = new anchor.BN(tokensToRaw(stakeAmount));

    //Fetch proposal counter to get the current proposal count for deriving the proposal PDA.
    const proposalCounterAccount = await program.account.proposalCounter.fetch(proposalCounterPda);

    // Deriving the Proposal account PDA using the current proposal count
    ...
    console.log("Proposal registered with transaction: ", tx);
  };
  return (
    <div className="card">
